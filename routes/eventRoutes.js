...
router
  .route("/")
  .post(authMiddleware, upload.single("image"), createEvent) // Middleware para cargar imagen en la creación
  .get(getEvents);
...
router
  .route("/:id")
  .get(getEvent)
  .put(authMiddleware, upload.single("image"), updateEvent) // Middleware para cargar imagen en la actualización
